...
# RAG Data Management System (New)

The project now includes a RAG (Retrieval-Augmented Generation) system to enhance the WhatsApp bot with store data (prices, stock, products).

## Running the System

To start the complete system (Backend API + React Frontend + WhatsApp Bot):

```bash
npm run dev
```

The server will kick off at `http://localhost:3000`.

## Features

- **Store Data Management**: Access the dashboard at `http://localhost:3000` to upload CSV, JSON, or TXT files.
- **AI Integration**: The WhatsApp bot automatically queries this data to answer customer questions.
...
4. Test the search in the "Test Search" tab
